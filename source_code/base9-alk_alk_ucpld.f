...
C  BECAUSE HAMMAT ASSUMES THAT IS WHERE THEY ARE.
...
C
      IF (ABS(M).EQ.INUCA+1) THEN
        EA=DBLE(INUCA)*HFSPLA/(2.D0*DBLE(INUCA+1))+SIGN(1.D0,DBLE(M))
...
C
      IF (ABS(M).EQ.INUCB+1) THEN
        EB=DBLE(INUCB)*HFSPLB/(2.D0*DBLE(INUCB+1))+SIGN(1.D0,DBLE(M))
